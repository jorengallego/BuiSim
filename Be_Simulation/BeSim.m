...
%     Y = ctrl.precomputed.Y(:,SimStart:SimStop)+273.15;   % infrax - signal in deg C
    ...
    SPF = refs.SPF(:,SimStart:SimStop+N);
...
%             TODO: predictions as standalone function
            ...
            SPF_prev = SPF(:, k:k+(ctrl.MPC.Nrp-1));
            
            
%             TODO:  adapt Dpreview wa_prev wb_prev
            if estim.use   % estimated states
                if model.plant.nd == 0  %  no disturbnances option
                     [opt_out, feasible, info1, info2] =  ctrl.MPC.optimizer{{xp, wa_prev, wb_prev, Price_prev, COP_prev, SPF_prev}}; % optimizer with estimated states
                else
                     [opt_out, feasible, info1, info2] =  ctrl.MPC.optimizer{{xp, Dpreview, wa_prev, wb_prev, Price_prev, COP_prev, SPF_prev}}; % optimizer with estimated states
                end
            else    % perfect state update
                if model.plant.nd == 0  %  no disturbnances option
                     [opt_out, feasible, info1, info2] =  ctrl.MPC.optimizer{{x0, wa_prev, wb_prev, Price_prev, COP_prev, SPF_prev}}; % optimizer with estimated states
                else
                     [opt_out, feasible, info1, info2] =  ctrl.MPC.optimizer{{x0, Dpreview, wa_prev, wb_prev, Price_prev, COP_prev, SPF_prev}}; % optimizer with measured states  
                end
